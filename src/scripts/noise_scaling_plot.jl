...
for i in eachindex(resolutions)
    ...
    cb.set_label(L"{\rm RMS\ RV\ (m\ s}^{-1} {\rm )}")
    ...
    ax2.set_xlabel(L"{\rm Per\ pixel\ SNR}", fontsize=16)
    ...
    ax1.set_title(L"{\rm Uncorrected\ RVs}", fontsize=18)
    ax2.set_title(L"{\rm Corrected\ RVs}", fontsize=18)
