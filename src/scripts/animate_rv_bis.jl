...
# bis = GRASS.moving_average(bis, 3)
# int = GRASS.moving_average(int, 3)

# create fig + axes objects
println("\t>>> Plotting one...")
fig, ax1 = plt.subplots(nrows=1, ncols=1, figsize=(6.075, 7.2))

# create lines
path_effects =[pe.Stroke(linewidth=5, foreground="k"), pe.Normal()]
l1 = ax1.plot([], [], c=c, path_effects=path_effects, ls="-", alpha=1.0, lw=2.0, zorder=1)[1]
...
function animate(i)
    ...
    l1.set_data(mean_bis .- mean(mean_bis) .+ delt_bis, int_i)

    # also plot the current one as new line
    ax1.plot(mean_bis .- mean(mean_bis) .+ delt_bis, int_i, alpha=0.1, c=c, zorder=0)#, rasterize=true)
    return l1
end

# write it out
println("\t>>> Writing animation one...")
frames = 1:2:Nt
writer = animation.PillowWriter(fps=10)
anim = animation.FuncAnimation(fig, animate, frames=frames, blit=false)
anim.save("/storage/home/mlp95/rvs_bis_full_res/bis_only_" * instrument[instr_idx] * ".gif", writer=writer, dpi=dpi)
plt.close("all")
...
path_effects =[pe.Stroke(linewidth=5, foreground="k"), pe.Normal()]
l1 = ax1.plot([], [], c=c, path_effects=path_effects, ls="-", alpha=1.0, lw=2.0, zorder=1)[1]
l2 = ax2.plot([], [], c=c, ls="none", marker="o", alpha=0.8, lw=2.0)[1]
ax2.plot(xmodel, ymodel, c="k", ls="--", label=L"{\rm R } \approx\ " * fit_label)
...
function animate(i)
    ...
    l1.set_data(mean_bis .- mean(mean_bis) .+ delt_bis, int_i)
    l2.set_data(xdata[1:i], ydata[1:i])
...
# write it out
println("\t>>> Writing animation two...")
frames = 1:2:Nt
writer = animation.PillowWriter(fps=10)
anim = animation.FuncAnimation(fig, animate, frames=frames, blit=false)
anim.save("/storage/home/mlp95/rvs_bis_full_res/rvs_bis_" * instrument[instr_idx] * ".gif", writer=writer, dpi=dpi)
plt.close("all")
...
ax1.axhline(int[idx55,1], ls=":", c="k")
ax1.axhline(int[idx90,1], ls=":", c="k")
ax2.plot(xmodel, ymodel, c="k", ls="--", label=L"{\rm R } \approx\ " * fit_label, zorder=1)
...
for i in 1:Nt
    ...
    # plot the last frame
    if i == Nt
        ax1.plot(mean_bis .- mean(mean_bis) .+ delt_bis, int_i, c=c, path_effects=path_effects, ls="-", alpha=1.0, lw=2.0, zorder=1)[1]
    end
end

fig.savefig("/storage/home/mlp95/rvs_bis_full_res/rvs_bis_" * instrument[instr_idx] * ".pdf")
plt.close("all")
